...
class TransportApp:
    ...
    def validate_weight(self, weight_str):
        ...
        try:
            ...
            if weight > 10000:
                return False, "Максимум 10000 кг"
            return True, weight
        except ValueError:
    ...
    def open_add_client(self, client=None):
        ...
        def save():
            ...
            if not name_valid:
                messagebox.showerror("Ошибка", f"Имя: {name_error}")
                name_entry.focus_set()
                return
            
            weight_valid, weight_result = self.validate_weight(weight_var.get())
            if not weight_valid:
                messagebox.showerror("Ошибка", f"Вес: {weight_result}")
    ...
    def open_add_vehicle(self, vehicle=None):
        ...
        def save():
            ...
            try:
                ...
                messagebox.showerror("Ошибка", "Некорректная грузоподъемность")
                return
    ...
    def update_client_table(self):
        ...
        for client in self.clients:
            vip_text = "✓" if client.get('vip', False) else ""
    ...
    def export_json(self):
        if not self.results:
            messagebox.showwarning("Нет данных", "Сначала выполните распределение")
            return
        filename = filedialog.asksaveasfilename(
            defaultextension=".json",
            filetypes=[("JSON файлы", "*.json")],
        ...
        if filename:
            ...
            self.status_bar.config(text="Результаты экспортированы в JSON")
    def export_txt(self):
        """Экспорт в TXT"""
        if not self.results:
            messagebox.showwarning("Нет данных", "Сначала выполните распределение")
